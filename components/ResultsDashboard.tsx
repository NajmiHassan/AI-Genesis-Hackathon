 ...
 * Shows the image, status, Notion save status, and the extracted data table.
 */
...
const ReceiptResultCard: React.FC<{ receipt: ProcessedReceipt }> = ({ receipt }) => {
  const getNotionStatusIcon = () => {
    switch (receipt.notionStatus) {
      case 'success':
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="flex items-center gap-1">
                {getNotionStatusIcon()}
                <span className="capitalize">{receipt.notionStatus}</span>
              </span>
            </div>
          </div>
          {receipt.error && (
